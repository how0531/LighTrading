    ...
    try:
        ...
        for t in trades:
            ...
            if status_name in active_statuses:
                # 處理外部期權委託 symbol 為空字串的問題，改抓 code
...
async def place_order(req: PlaceOrderRequest):
    ...
    if trade:
        ...
        return {"status": "success", "message": "下單成功", "working_orders": snapshot}
    else:
...
async def cancel_all(req: CancelAllRequest):
    ...
    try:
        ...
        # 刪單後立即取得已確認的委託快照
        snapshot = await _get_working_orders_snapshot()
        return {"status": "success", "message": f"成功送出 {cancel_count} 筆刪單指令", "working_orders": snapshot}
    except Exception as e:
