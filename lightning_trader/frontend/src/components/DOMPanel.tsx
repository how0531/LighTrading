...
const DOMPanel: React.FC = () => {
  ...
  // 委託單查詢（由 TradingContext 統一管理）
  // workingOrders 和 refreshOrders 已經從 context 取得，不再在此處輪詢
  ...
  const handlePlaceOrder = useCallback(async (price: number, action: 'Buy' | 'Sell') => {
    // 只用 isOrderPendingRef 防止重複送單；isStale 僅為 UI 顯示，不阻擋下單
  ...
  }, [targetSymbol, orderValue, orderType, splitCfg, setWorkingOrders]);
  ...
  const handleCancelOrder = useCallback(async (action: 'Buy' | 'Sell', price?: number) => {
    try {
      let finalRes;
      if (price !== undefined) {
        // 從 workingOrders 找出指定價格的實際合約代碼（可能是 TXFC8 等完整代碼）
        const pKey = Math.round(price * 100);
        const code = targetSymbol.replace(/\D/g, '');
        ...
        // 對每個唯一的合約代碼送出 cancel_all（確保命中正確的合約）
        const symbols = [...new Set(matchedOrders.map((o: WorkingOrder) => o.symbol))];
        const targets = symbols.length > 0 ? symbols : [targetSymbol];
        const results = await Promise.all(targets.map(sym =>
          apiClient.post('/cancel_all', { symbol: sym, action })
        ));
        // 取最後一個回傳結果更新
        if (results.length > 0) finalRes = results[results.length - 1];
      } else {
        // 全刪（Footer 按鈕）：發送給 targetSymbol
        finalRes = await apiClient.post('/cancel_all', { symbol: targetSymbol, action });
      }
      // ★ 嚴格依賴後端：只使用後端回傳的確認快照
      if (finalRes?.data?.working_orders) {
        setWorkingOrders(finalRes.data.working_orders);
      ...
      console.error('[DOMPanel] handleCancelOrder 失敗:', e);
    }
          ...
          <div className="flex items-center gap-2">
            ...
            {(qData.AvgPrice ?? 0) > 0 && (
              <div className="flex flex-col items-center px-2 py-1 rounded border border-slate-700 bg-slate-800/60">
                ...
                <span className={`text-[12px] font-mono font-black tabular-nums leading-none ${currentPrice > (qData.AvgPrice ?? 0) ? 'text-red-400' :
