...
// Sprint 34：報價看板 + 多圖看盤 lazy
const QuoteBoardPanel       = lazy(() => import('./QuoteBoardPanel'));
...
// Sprint 34：新增 quotes / mchart 兩面板，bump key（既有 layout 會被替換為含新面板的預設）
const LAYOUT_KEY = 'lighTrade_layout_v9';

// react-grid-layout 會把定位 props / ref 注入「直接子節點」,因此 grid item 必須是
// 原生 div（不能用自訂 component 包,否則 style/ref 不會落到 DOM 上）。這個 helper
// 只回傳「div 內部」要共用的內容（drag handle + flex 容器）,div 本身仍寫在 JSX。
const dragHandle = (
  <div className="drag-handle bg-slate-700/80 hover:bg-slate-700 text-center py-1 text-xs text-slate-300 cursor-move tracking-widest uppercase font-bold transition-colors">DRAG</div>
);
const itemClass = (locked: boolean) =>
  `flex flex-col overflow-hidden rounded-lg ${!locked ? 'ring-1 ring-slate-500 bg-slate-800/20' : ''}`;
